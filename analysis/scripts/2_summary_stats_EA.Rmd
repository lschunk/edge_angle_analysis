...
ID <- summaryBy(. ~ ID+Tool_type+Edge+Section+Distance_origin, 
                   data = imp_data[c("ID", "Tool_type", "Edge", "Section", "Distance_origin",
                                   names(imp_data)[num.var])], 
                   FUN = nminmaxmeanmedsd)
str(ID)
```
...
site <- summaryBy(. ~ Site+ID+Tool_type+Edge+Section+Distance_origin, 
                            data = imp_data[c("Site", "ID", "Tool_type", "Edge", "Section", "Distance_origin",
                                           names(imp_data)[num.var])],
                          FUN = nminmaxmeanmedsd)
str(site)
```
...
## Write to XLSX
```{r}
write.xlsx(list(edge = edge, tool_sec = tool_sec, tool_dist = tool_dist, tool_sec_dist = tool_sec_dist, ID = ID, dist = dist, sec =sec, site = site), 
...
## Save R object
```{r}
saveObject(list(edge = edge, tool_sec = tool_sec, tool_dist = tool_dist, ID = ID, dist = dist, sec =sec, site = site), 
